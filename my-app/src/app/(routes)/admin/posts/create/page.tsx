...
function PostEditor() {
  ...
  const handlePublish = async () => {
    ...
    // Publish confirmation toast modal
    toast(
      (t) => (
        <div className="bg-white rounded-xl shadow-2xl border border-gray-100 p-6 max-w-sm">
          <div className="flex items-center mb-4">
            ...
            <div className="ml-3">
              <h3 className="text-lg font-semibold text-gray-900">記事の公開</h3>
            </div>
          </div>
          
          <p className="text-gray-600 mb-6 leading-relaxed">
            「{titleEditor?.getText().trim()}」を公開しますか？
          </p>
          ...
          <div className="flex justify-end space-x-3">
            ...
            <button
              onClick={() => { doPublish(); toast.dismiss(t.id); }}
              className="px-4 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-lg font-medium transition-all duration-200 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-offset-2 shadow-lg hover:shadow-xl"
            >
  ...
  const handleClose = () => {
    if (titleEditor?.getText() || editor?.getText()) {
      const doClose = () => {
        router.push('/admin/posts');
      };
                  ...
                  {/* List */}
                   <button
                     onClick={() => {
                       ...
                       console.log('After toggle bullet list state:', editor?.isActive('bulletList'));
                     }}
                     className={`p-1.5 rounded transition-colors ${
