...
export default function AdminRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (emailData.error) {
        setMessage(emailData.error);
        setMessageType('error');
      } else if (emailData.message === '認証メールは既に送信済みです') {
        setMessage('メールを送信済みです');
        setMessageType('error');
      } else if (emailData.message === 'メール送信に成功しました') {
        setMessage('認証メールを送信しました');
        setMessageType('success');
      } else {
        setMessage('登録情報は既に存在しています');
        setMessageType('error');
      }
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                autoComplete="email"
                required
                className="mt-1 appearance-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-300 text-gray-900 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
